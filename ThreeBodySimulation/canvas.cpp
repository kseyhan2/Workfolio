...
{
public:
    canvas(std::string filename, double scaleX=1, double scaleY=1, int offsetX=centerX, int offsetY=centerY);
...
{
    if(newfile)
        outputFile.open(filename.c_str()); // c_str() eklenmeli
    else
        outputFile.open(filename.c_str(), std::ofstream::app); // c_str() eklenmeli
}
